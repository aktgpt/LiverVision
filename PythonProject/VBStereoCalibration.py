...
class StereoCalibration:
    ...
    def getImagesFromFolder(self):
        ...
        for i in range(len(images_left)):
            imgs_l = imgs_l.append(cv2.imread(images_left[i]))
            imgs_r = imgs_r.append(cv2.imread(images_right[i]))
    ...
    def readImages(self):
        for i in range(len(self.imagesForCalibration[0])):
            img_l = cv2.imread(self.imagesForCalibration[0][i])
            img_r = cv2.imread(self.imagesForCalibration[1][i])
    ...
    def getCalibrationParameters(self):
        ...
        self.StereoParameters = dict([('FundamentalMatrix', F), ('LeftProjectionMatrix', extrinsicParams[2]),
                                      ('RightProjectionMatrix', extrinsicParams[3])])

    def saveCalibrationParameters(self):
        np.savez("StereoParameters", FundamentalMatrix=self.StereoParameters['FundamentalMatrix'],
    ...
    def loadCalibrationParameters(self):
        if os.path.isfile(self.filesPath + "StereoParameters.npz"):
            ...
            self.StereoParameters = dict([('FundamentalMatrix', calibrationParameters['FundamentalMatrix']),
